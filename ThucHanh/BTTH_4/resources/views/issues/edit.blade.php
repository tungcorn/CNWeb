...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            {{-- Form Start --}}
            <form action="{{ route('issues.update', $issue) }}" method="POST" onsubmit="this.submitBtn.disabled = true;">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('reported_by') is-invalid @enderror"
                               id="reported_by" name="reported_by" value="{{ old('reported_by', $issue->reported_by ?? '' ) }}"
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" class="form-control @error('reported_date') is-invalid @enderror"
                               id="reported_date" name="reported_date" value="{{ old('reported_date', $issue->reported_date->format('Y-m-d\TH:i') ?? '' ) }}">
                        @error('reported_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror"
                              id="description" name="description" rows="5" >{{ old('description', $issue->description ?? '' ) }}</textarea>
                    @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('urgency') is-invalid @enderror"
                            ...
                            <option value="" selected disabled>Select a Urgency</option>
                            <option value="Low" {{ old('urgency', $issue->urgency ?? '') == 'Low' ? 'selected' : '' }}>Low
                            </option>
                            <option value="Medium" {{ old('urgency', $issue->urgency ?? '') == 'Medium' ? 'selected' : '' }}>Medium
                            </option>
                            <option value="High" {{ old('urgency', $issue->urgency ?? '') == 'High' ? 'selected' : '' }}>High
                            </option>
                        </select>
                        @error('urgency')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror"
                            ...
                            <option value="Open" {{ old('status', $issue->status ?? '') == 'Open' ? 'selected' : '' }}>Open
                            </option>
                            <option value="In Progress" {{ old('status', $issue->status ?? '') == 'In Progress' ? 'selected' : '' }}>In Progress
                            </option>
                            <option value="Resolved" {{ old('status', $issue->status ?? '') == 'Resolved' ? 'selected' : '' }}>Resolved
                            </option>
                        </select>
                        @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
